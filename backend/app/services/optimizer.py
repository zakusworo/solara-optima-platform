...
class UCEDOptimizer:
    ...
    def _create_objective(self, T: int, G: int, request: OptimizationRequest):
        ...
        for g in generators:
            ...
            for t in time_periods:
                ...
                if gen.emissions_rate > 0 and settings.CARBON_PRICE > 0:
                    carbon_cost = (
                        gen.emissions_rate * settings.CARBON_PRICE / 1000
        ...
        if request.bess_capacity > 0:
            for t in time_periods:
                degradation = (
                    self.variables["p_bess_ch"][t] + self.variables["p_bess_dis"][t]
        ...
        if request.tou_prices:
            for t in time_periods:
                grid_cost = (
                    request.tou_prices[t] * self.variables.get("p_grid", [0] * T)[t]
                    if "p_grid" in self.variables
                    else 0
                )
    ...
    def _create_constraints(self, T: int, G: int, request: OptimizationRequest):
        ...
        for g in generators:
            ...
            for t in time_periods:
                ...
                self.model += (
                    self.variables["p"][(g, t)]
                    >= gen.min_output * self.variables["u"][(g, t)],
                    f"MinOutput_{g}_{t}",
                )
                self.model += (
                    self.variables["p"][(g, t)]
                    <= gen.max_output * self.variables["u"][(g, t)],
                    f"MaxOutput_{g}_{t}",
                )
                ...
                if t > 0:
                    self.model += (
                        self.variables["p"][(g, t)] - self.variables["p"][(g, t - 1)]
                        <= gen.ramp_up,
                        f"RampUp_{g}_{t}",
                    )
                else:
                ...
                if t > 0:
                    self.model += (
                        self.variables["p"][(g, t - 1)] - self.variables["p"][(g, t)]
                        <= gen.ramp_down,
                        f"RampDown_{g}_{t}",
                    )
                else:
                    self.model += (
                        gen.initial_output - self.variables["p"][(g, t)]
                        <= gen.ramp_down,
                        f"RampDown_{g}_{t}",
                    )
        ...
        for t in time_periods:
            ...
            self.model += (
                pl.lpSum(reserve_terms) >= reserve_required,
                f"SpinningReserve_{t}",
            )
        ...
        if request.bess_capacity > 0 and "soc" in self.variables:
            for t in time_periods:
                ...
                self.model += (
                    ...
                    <= request.bess_power_rating * self.variables["bess_mode"][t],
                    f"ChargeMode_{t}",
                )
                self.model += (
                    self.variables["p_bess_dis"][t]
                    <= request.bess_power_rating * (1 - self.variables["bess_mode"][t]),
                    f"DischargeMode_{t}",
                )
            ...
            if request.bess_final_soc is not None:
                self.model += (
                    self.variables["soc"][T - 1]
                    >= request.bess_capacity * request.bess_final_soc,
                    "FinalSOC",
                )
        ...
        if gen.min_uptime > 1:
            for t in range(T):
                if t >= gen.min_uptime - 1:
                    ...
                    self.model += (
                        pl.lpSum(
                            self.variables["v"][(g, tau)]
                            for tau in range(t - gen.min_uptime + 1, t + 1)
                        )
        ...
        if gen.min_downtime > 1:
            for t in range(T):
                if t >= gen.min_downtime - 1:
                    ...
                    self.model += (
                        pl.lpSum(
                            self.variables["w"][(g, tau)]
                            for tau in range(t - gen.min_downtime + 1, t + 1)
                        )
    ...
    def solve(self) -> Optional[OptimizationResult]:
        ...
        logger.info(
            f"Solving with {self.solver_name} solver (time limit: {self.time_limit}s)"
        )
        ...
        if self.model.status != pl.LpStatusOptimal:
            logger.warning(
                f"Optimization did not converge to optimal solution: {status}"
            )
    ...
    def _extract_solution(self) -> Optional[OptimizationResult]:
        ...
        if "p_solar" in self.variables:
            solar_output = [
                pl.value(self.variables["p_solar"][t]) or 0 for t in range(T)
            ]
        ...
        if "soc" in self.variables:
            battery_operation = {
                "charge": [
                    pl.value(self.variables["p_bess_ch"][t]) or 0 for t in range(T)
                ],
                "discharge": [
                    pl.value(self.variables["p_bess_dis"][t]) or 0 for t in range(T)
                ],
        ...
        fuel_cost = sum(
            (pl.value(self.variables["p"][(g, t)]) or 0)
            * self.request.generators[g].fuel_cost
            for g in range(G)
            for t in range(T)
        )
        ...
        total_emissions = sum(
            (pl.value(self.variables["p"][(g, t)]) or 0)
            * self.request.generators[g].emissions_rate
            for g in range(G)
            for t in range(T)
        )

        logger.info(
            f"Cost breakdown — total: {total_cost:.0f}, fuel: {fuel_cost:.0f}, emissions: {total_emissions:.1f} kg CO2"
        )
        ...
        return OptimizationResult(
            ...
            load_served=[
                pl.value(self.model.constraints[f"PowerBalance_{t}"].constant) or 0
                for t in range(T)
            ],
