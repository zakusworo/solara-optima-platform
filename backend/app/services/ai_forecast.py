...
class AIForecastingService:
    ...
    def __init__(
        ...
        logger.info(
            f"AIForecastingService initialized with {self.model} at {self.host}"
        )
    ...
    def generate_load_forecast(
        ...
        logger.info(
            f"Generating AI load forecast: {len(historical_data)} historical points, {horizon_hours}h horizon"
        )
        ...
        try:
            ...
            forecast_values = self._parse_forecast_response(
                forecast_text, horizon_hours
            )
            ...
            logger.info(
                f"AI load forecast generated: {len(forecast_values)} values, confidence={confidence:.2f}"
            )
            return result
...
Forecast:"""
    ...
    def _fallback_forecast(
        ...
        if len(historical) >= 24:
            ...
            for h in range(24):
                hour_values = [
                    historical[i] for i in range(h, min(len(historical), 168), 24)
                ]
  ...
  "reasoning": "brief explanation"
    ...
    def compare_forecasting_methods(
        ...
        for h in range(horizon_hours):
            hour_values = [
                historical_data[i] for i in range(h, len(historical_data), 24)
            ]
