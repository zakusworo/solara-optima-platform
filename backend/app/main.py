...
def create_application() -> FastAPI:
    ...
    if not allowed_origins:
        logger.warning(
            "CORS_ORIGINS is empty; no cross-origin requests will be accepted"
        )
    ...
    application.include_router(
        optimize.router, prefix="/api/v1/optimize", tags=["Optimization"]
    )
    application.include_router(
        forecast.router, prefix="/api/v1/forecast", tags=["Forecasting"]
    )
    application.include_router(
        generators.router, prefix="/api/v1/generators", tags=["Generators"]
    )
    application.include_router(
        weather.router, prefix="/api/v1/weather", tags=["Weather"]
    )
    application.include_router(
        ai_forecast.router, prefix="/api/v1/ai", tags=["AI Forecasting"]
    )
    application.include_router(
        location.router, prefix="/api/v1/location", tags=["Location"]
    )
    application.include_router(
