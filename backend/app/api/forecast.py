...
from app.models.schemas import APIResponse, ForecastRequest, LoadForecastResponse
...
async def get_solar_forecast(
    ...
    hours: int = Query(
        default=24, ge=1, le=168, description="Forecast horizon (hours)"
    ),
...
async def get_load_forecast(
    ...
    try:
        ...
        logger.info(
            f"Load forecast: peak={peak_load:.1f}kW, total={total_energy:.1f}kWh"
        )
...
async def create_custom_load_forecast(request: ForecastRequest):
    ...
    try:
        ...
        logger.info(
            f"Creating custom load forecast from {len(request.historical_data)} data points"
        )
        ...
        for h in range(hours_in_day):
            hour_values = [
                historical[i] for i in range(h, len(historical), hours_in_day)
            ]
            hourly_avg.append(sum(hour_values) / len(hour_values) if hour_values else 0)

        # Generate forecast
        forecast_hours = int(
            (request.end_date - request.start_date).total_seconds() / 3600
        )
        timestamps = []
        load_values = []
...
async def compare_scenarios(
    ...
    improvement = (
        ...
        / forecast_fixed["total_generation"]
        * 100
    )
